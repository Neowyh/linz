...
export const browserTool = new DynamicStructuredTool({
  ...
  description:
    ...
    '动作列表：navigate 打开网页（参数 url，仅支持 http/https）；click 点击元素（参数 selector，CSS 选择器）；' +
    'type 在输入框输入文本（参数 selector + text）；scroll 滚动页面（参数 deltaY 像素，或给 selector 滚动到该元素）；' +
    'back / forward / reload 后退 / 前进 / 刷新；screenshot 截图（结果直接以图片显示在对话中）；' +
    'read 读取当前页面的标题、网址和正文文本；wait 等待元素出现（参数 selector + timeoutMs）。' +
    '操作作用于浏览器当前激活的标签页，首次调用会自动打开浏览器面板。' +
    '注意：click/type/scroll 作用于页面主框架文档，无法操作跨域 iframe 内部元素；' +
    '元素不存在或选择器写错会返回错误提示，可根据提示调整选择器后重试；页面未加载完时可用 wait 等待元素出现。',
  schema: z.object({
    action: z
      .enum(['navigate', 'click', 'type', 'scroll', 'back', 'forward', 'reload', 'screenshot', 'read', 'wait'])
      .describe('要执行的操作'),
    url: z.string().optional().describe('navigate 的目标网址，仅支持 http/https'),
    selector: z
      .string()
      .optional()
      .describe('CSS 选择器（click/type/scroll 到元素/wait 使用），如 #search-input、input[name=q]、button[type=submit]'),
    text: z.string().optional().describe('type 时要输入的文本'),
    deltaY: z.number().optional().describe('scroll 垂直滚动像素（正数向下，默认 600；提供 selector 时改为滚动到该元素）'),
