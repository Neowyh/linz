...
export class OfficeScene {
  ...
  private agents: Agent[] = getInitialAgents().map((a) => ({ ...a }))
  private readonly options: {
    onAgentClick?: (event: OfficeAgentClick) => void
  }

  /** 适配画布的基准缩放（wheel 缩放下限） */
  private baseScale = 1
  ...
  private setupInteraction() {
    ...
    this.onWheel = (event: WheelEvent) => {
      ...
      const next = Math.max(this.baseScale, Math.min(this.baseScale * 4, cur * factor))
      if (next === cur) return

      // 以指针为锚点缩放：保持光标下的世界坐标不动
      const wx = (px - this.world.x) / cur
      const wy = (py - this.world.y) / cur
    ...
    const isAgentTarget = (target: unknown): boolean => {
      let node = target as { parent?: unknown; agentId?: unknown } | null
      while (node) {
        if (node.agentId !== undefined) return true
        node = (node.parent as { parent?: unknown; agentId?: unknown } | null) ?? null
      }
      return false
    }

    const onPointerDown = (event: FederatedPointerEvent) => {
      if (isAgentTarget(event.target)) return
      this.dragging = true
      this.dragStart = {
        x: event.globalX,
        y: event.globalY,
    ...
    const onPointerMove = (event: FederatedPointerEvent) => {
      ...
      this.world.x = this.dragStart.worldX + (event.globalX - this.dragStart.x)
      this.world.y = this.dragStart.worldY + (event.globalY - this.dragStart.y)
    }
    const onPointerUp = () => {
  ...
  resize(containerWidth: number, containerHeight: number) {
    ...
    if (this.app) this.app.stage.hitArea = this.app.screen
  }
