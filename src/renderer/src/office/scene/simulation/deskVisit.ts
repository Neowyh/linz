...
export function rosterAgentAt(agents: Agent[], rosterNo: number): Agent | undefined {
  const i = rosterNo - 1
...
export function deskForAgent(agent: Agent): Desk {
  ...
  return (
    ...
    desks[roster.findIndex((r) => r.id === agent.id)] ??
    desks[0]
  )
}
...
function buildStops(
  ...
  for (const rosterNo of hostRosterNos) {
    ...
    if (!host) continue
    const name = roster[rosterNo - 1]?.name ?? `#${rosterNo}`
    stops.push({
...
function syncDeskVisitTalkPartners(agents: Agent[]): Agent[] {
  ...
  result = result.map((agent) => {
    ...
    const mission = agent.mission
    const rEntry = roster.find((r) => r.id === agent.id)
    ...
    if (mission?.kind === 'desk_visit' && mission.phase !== 'talk') {
      ...
      return {
        ...
        currentTask: mission.resumeTask ?? rEntry?.task ?? agent.currentTask,
        bubbleText: undefined,
      }
    }
    ...
    if (agent.state === 'talking' && !agent.mission && !agent.customAnimation) {
      return {
        ...
        state: 'working' as const,
        viewFacing: 'back' as const,
        currentTask: rEntry?.task ?? agent.currentTask,
        bubbleText: undefined,
      }
    }
